...
THIS CODE WAS MODIFIED BY MAXMIMILIAN BAKER
E-mail: bakermk@cardiff.ac.uk
All rights to above Michele Cappellari

CHANGELOG
- Modified _sn_func to return TOTAL particle count, and total velocity count for a given bin
- Code now takes a 'target number of particles' rather than target sn, 
  and will attempt to return bins with minimum particle counts above this value
- Input for signal = mass-weighted mean velocity from 2dhist
- Input for noise = particle count from 2dhist

This code will produce square/rectangular bins of data corresponding to the required particle count
This can then be visualised through a built-in voronoi function, but these visual polygons 
MAY not contain the min. number of particle counts, as we are taking rectangular data and
visualising it with a voronoi
...
def _accretion(x, y, signal, noise, target_sn, pixelsize, quiet, sn_func):
    ...
    #
    w = noise < target_sn
    maxnum = int(np.sum((noise[w])**2)/target_sn**2 + np.sum(~w))
    ...
    for it in range(xnode.size):  # Do at most xnode.size iterations
        ...
        if wvt:
            for k in good:
                ...
                sn, vel = sn_func(index, signal, noise)
...
def voronoi_2d_binning(x, y, signal, noise, target_sn, cvt=True, pixelsize=None,
    ...
    noise: array_like with shape (npix,)
        ...
        CHANGE:
        Change to take in 2dhist bin (particle) counts               
    target_sn: float
