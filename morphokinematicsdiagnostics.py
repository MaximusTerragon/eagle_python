...
def kinematics_diagnostics(XYZ, mass, Vxyz, aperture=0.03, CoMvelocity=True):
    """ 
    ...
    return kappa, delta, discfrac, vrotsig
...
def morphological_diagnostics(XYZ, mass, Vxyz, aperture=0.03, CoMvelocity=True, reduced_structure=True):
    ...
    while not('structure' in locals()) or (reduced_structure and not(stop)):
        ...
        structmainaxe *= sign
        temp = np.array([1,sign,1])*(eigvec[:,(np.argmin(eigval)+np.array([(3+sign)/2,0,(3-sign)/2]).astype(int))%3])
        eigval = eigval[(np.argmin(eigval)+np.array([(3+sign)/2,0,(3-sign)/2]).astype(int))%3]
