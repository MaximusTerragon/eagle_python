...
# Will plot sample selection given criteria plus SFR plot
def _plot_stellar_mass_function_plus_sfr(csv_sample = 'L100_27_all_sample_misalignment_9.5',     # CSV sample file to load GroupNum, SubGroupNum, GalaxyID, SnapNum
                                csv_output = '_RadProj_Err__stars_gas_stars_gas_sf_gas_sf_gas_nsf_stars_dm_gas_dm_gas_sf_dm_',
                                 #--------------------------
                                 # What determines our final sample
                                 print_summary = True,
                                   ...
                                   use_proj_angle      = True,                   # Whether to use projected or absolute angle 10**9
                                     ...
                                     lower_mass_limit  = 10**9.5,            # Lower limit of chosen sample
                                     upper_mass_limit  = 10**15,            # Lower limit of chosen sample
                                   pop_mass_limit      = 10**9.4,            # Lower limit of population plot sampled
                                   use_satellites      = True,   
                                 #--------------------------
                                 hist_bin_width = 0.1,
                                 #--------------------------
    ...
    def _plot_stellar_mass_func(debug=False):
        ...
        # Graph initialising and base formatting
        #fig, (ax_top, ax_bot) = plt.subplots(2, 1, figsize=[10/3, 4], sharex=True, sharey=False)
        ...
        for sim_name, sim_size in sample_input['mySims']:
            ...
            ax_top.plot(myData['mass'], np.log10(hist), label=sim_name, linewidth=1, c='k')
        ...
        # Create histogram of our sample    
        hist_sample, _ = np.histogram((hist_bin_width/2)+np.floor(np.log10(sample_dict['stelmass'])/hist_bin_width)*hist_bin_width , bins=np.arange(np.log10(mod_lower_mass_limit)+(hist_bin_width/2), np.log10(upper_mass_limit), hist_bin_width))
        hist_sample = hist_sample[:] / (float(sample_input['mySims'][0][1]))**3
        hist_sample = hist_sample / hist_bin_width                      # why?
        hist_bins   = np.arange(np.log10(mod_lower_mass_limit)+(hist_bin_width/2), np.log10(upper_mass_limit)-hist_bin_width, hist_bin_width)
        ...
        #=========================================
        # plot line hist
        ax_top.plot(hist_bins, np.log10(hist_sample), label='Sample selection', ls='--', linewidth=1, c='C0')
        
        #----------
        # Contours for L100 sample
        sns.kdeplot(x=np.log10(L100_dict['stelmass']), y=np.log10(L100_dict['sfr']), ax=ax_bot, levels=5, color='k', fill=False, linewidths=1, alpha=0.7)
        
        
        #-----------
        # Scatter
        #ax_bot.scatter(np.log10(L100_dict['stelmass']), np.log10(L100_dict['sfr']), s=1, c='k', zorder=-5, alpha=0.5, label=sample_input['mySims'][0][0])
        ax_bot.scatter(np.log10(sample_dict['stelmass']), np.log10(sample_dict['sfr']), s=0.7, c='C0', zorder=1, alpha=0.5, edgecolors='none', label='Sample selection')
        
        
        
        #-----------
        # Axis formatting
        ax_top.set_xlim(9.5, 12)
        ax_bot.set_xlim(9.5, 12)
        ax_top.set_ylim(-5, -1.5)
        ax_top.set_yticks(np.arange(-5, -1.4, 0.5))
        ax_bot.set_yticks(np.arange(-2, 2, 0.5))
        ax_top.set_xticklabels([])
        ax_bot.set_xlabel(r'log$_{10}$ $M_{*}$ [M$_{\odot}$]')
        ax_top.set_ylabel(r'log$_{10}$ dn/dlog$_{10}$($M_{*}$)'+'\n'+'[cMpc$^{-3}$]')
        ax_bot.set_xticks(np.arange(9.5, 12.5, 0.5))
        ax_bot.set_ylim(-2, 1.5)
        ax_bot.set_ylabel(r'log$_{10}$ SFR [M$_{\odot}$yr$^{-1}$]')
        ...
        ax_top.legend(handles=legend_elements, labels=legend_labels, loc='upper right', frameon=False, labelspacing=0.1, labelcolor=legend_colors, handlelength=0, ncol=1)
        ...
        if savefig:
            plt.savefig("%s/sample_stellar_mass_function/L%s_%s_%s_misalignment_%s_m%sm%s_HMR%s_proj%s_inc%s_stellar_mass_func_plus_sfr_%s.%s" %(fig_dir, output_input['mySims'][0][1], output_input['snapNum'], sat_str, np.log10(float(output_input['galaxy_mass_min'])), np.log10(float(output_input['galaxy_mass_max'])), use_angle, str(use_hmr), use_proj_angle, min_inc_angle, savefig_txt, file_format), metadata=metadata_plot, format=file_format, bbox_inches='tight', dpi=600)    
            print("\n  SAVED: %s/sample_stellar_mass_function/L%s_%s_%s_misalignment_%s_m%sm%s_HMR%s_proj%s_inc%s_stellar_mass_func_plus_sfr_%s.%s" %(fig_dir, output_input['mySims'][0][1], output_input['snapNum'], sat_str, np.log10(float(output_input['galaxy_mass_min'])), np.log10(float(output_input['galaxy_mass_max'])), use_angle, str(use_hmr), use_proj_angle, min_inc_angle, savefig_txt, file_format))
        if showfig:
            plt.show()
        plt.close()
...
# plots as before but adds sfr plot
_plot_stellar_mass_function_plus_sfr(use_angle = 'stars_gas_sf', 
                                showfig       = True,
                                savefig       = True)
