...
class Subhalo_Extract:
    ...
    def __init__(self, sim, gn, sgn, load_region_length=2):       # cMpc/h
        ...
        self.centre       = 1000*np.array([myData['x'], myData['y'], myData['z']])                          # [pkpc]
        self.centre_mass  = 1000*np.array([myData['x_mass'], myData['y_mass'], myData['z_mass']])           # [pkpc]
        self.perc_vel     = np.array([myData['vel_x'], myData['vel_y'], myData['vel_z']]) * u.km.to(u.kpc)  # [pMpc/s]
